...
docker run  --name nginx-md-{proxyName}-proxy -v `pwd`/targetservers-build:/etc/nginx/targetservers -t nginx-md-{proxyName}-proxy bash /etc/nginx/test.sh

#Artefakte für md-{proxyName} und km-{proxyName} bauen
